...
public class Hash {
	...
	public void add(String input) {
		...
		if(names[hashValue] == null)          //if array is free
		{
		...
		{
			int x = (hashValue+1 % names.length);    //linear traversal applied

			int count=0;
			while(true)
			{
				if(names[x] == null)  //agar index ke next ka element vacant ho to
				{
					names[x] = new String(input);   
					System.out.println("input String="+ input + "; got hashValue=" + hashValue + " and added at index=" + x);
					break;
				}
				x = (++x % names.length);//to add same index element we create an array of single element
				count++;                 //agar next place vacant nasel to uske next place

				if(count == names.length)   //to tell user that index is full
				{
